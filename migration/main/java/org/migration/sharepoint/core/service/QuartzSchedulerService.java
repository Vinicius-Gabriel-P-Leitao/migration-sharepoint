...
public class QuartzSchedulerService {
    ...
    private Trigger buildTrigger(MigrationJob job) {
        TriggerBuilder<Trigger> builder = TriggerBuilder.newTrigger()
                .withIdentity("trigger-" + job.getId(), "migration")
                .startNow();
    ...
    private Trigger buildIntervalTrigger(TriggerBuilder<Trigger> builder, MigrationJob job) {
        ...
        SimpleScheduleBuilder schedule = switch (job.getIntervalUnit()) {
            case MINUTES -> SimpleScheduleBuilder.simpleSchedule()
                    .withIntervalInMinutes(value).repeatForever();
            case HOURS -> SimpleScheduleBuilder.simpleSchedule()
                    .withIntervalInHours(value).repeatForever();
            case DAYS -> SimpleScheduleBuilder.simpleSchedule()
                    .withIntervalInHours(value * 24).repeatForever();
        };
