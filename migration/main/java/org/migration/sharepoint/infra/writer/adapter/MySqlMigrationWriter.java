...
public class MySqlMigrationWriter implements MigrationWriter {
    ...
    private static final List<String> MYSQL_NATIVE_TYPES = List.of("TINYINT",
            "SMALLINT",
            "INT",
            "BIGINT",
            "FLOAT",
    ...
    private static final Map<ColumnType, String> MYSQL_CANONICAL_MAP = Map.of(ColumnType.TEXT,
            "TEXT",
            ColumnType.NUMBER,
            "BIGINT",
            ColumnType.DECIMAL,
    ...
    @Override
    public void write(String connectionString, String targetName, List<Map<String, Object>> rows, Map<String, FieldMapping> columnTypes) {
    ...
    private void createTableIfAbsent(Connection conn, String catalog, String tableName, List<String> columns, Map<String, FieldMapping> columnTypes) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            String columnDefs = columns.stream().map(col -> {
                try {
                    String sqlType = columnTypes.containsKey(col)
    ...
    private String resolveType(String column, FieldMapping mapping) {
        ...
        if (mapping.nativeType() != null && !mapping.nativeType().isBlank()) {
            ...
            boolean valid = MYSQL_NATIVE_TYPES.stream().anyMatch(supported -> supported.split("\\(")[0].equalsIgnoreCase(inputBase));
            if (!valid) {
                throw new BadRequestException(ErrorCode.BAD_REQUEST,
                        "Tipo nativo inválido para MySQL na coluna '%s': '%s'. Tipos suportados: %s".formatted(column,
                                mapping.nativeType(),
                                MYSQL_NATIVE_TYPES));
            }
        ...
        throw new BadRequestException(ErrorCode.BAD_REQUEST,
                "Coluna '%s' sem tipo definido: informe 'type' (canônico) ou 'nativeType' (nativo MySQL)".formatted(column));
    }

    private void replaceAll(Connection conn, String tableName, List<String> columns, List<Map<String, Object>> rows, Map<String, FieldMapping> columnTypes) throws SQLException {
    ...
    private void batchInsert(Connection conn, String tableName, List<String> columns, List<Map<String, Object>> rows, Map<String, FieldMapping> columnTypes) throws SQLException {
        try (Statement helper = conn.createStatement()) {
            String quotedTable = helper.enquoteIdentifier(tableName, true);

            String colList = columns.stream().map(col -> {
                try {
    ...
    private Object convertValue(Object value, FieldMapping mapping) {
        ...
        String raw = value instanceof String s
                ? s
                : null;
        if (raw == null) return value;

        ColumnType type = mapping.type();
        String nativeType = mapping.nativeType() != null
                ? mapping.nativeType().toUpperCase()
                : null;

        boolean isDatetime =
                type == ColumnType.DATETIME || (nativeType != null && (nativeType.startsWith("DATETIME") || nativeType.startsWith("TIMESTAMP")));
        boolean isDate = !isDatetime && (type == ColumnType.DATE || (nativeType != null && nativeType.startsWith("DATE")));
