 ...
 *
 * <p>O token é obtido via POST para o endpoint do tenant e cacheado em memória. É renovado
 * automaticamente 60 segundos antes de expirar.
 *
...
public class GraphClient {
  ...
  private GraphResponse fetch(String url, String siteId, String listId) {
    try {
      ...
      throw switch (httpClientError.getStatusCode().value()) {
        ...
        case 403 ->
            new InfrastructureException(
                ...
                "Sem permissão para acessar site='%s' list='%s' — verifique as permissões da App Registration (HTTP 403)"
                    .formatted(siteId, listId));
        case 404 ->
            new InfrastructureException(
                ErrorCode.GRAPH_SITE_OR_LIST_NOT_FOUND,
                "Site ou lista não encontrado: site='%s' list='%s' (HTTP 404)"
                    .formatted(siteId, listId));
        case 429 ->
            new InfrastructureException(
      ...
      throw new InfrastructureException(
          ...
          "Graph API indisponível (HTTP %d): %s"
              .formatted(httpServerError.getStatusCode().value(), httpServerError.getMessage()));

    } catch (ResourceAccessException networkError) {
      throw new InfrastructureException(
          ErrorCode.GRAPH_TIMEOUT,
          "Timeout ou falha de rede ao acessar a Graph API: %s"
              .formatted(networkError.getMessage()));
      ...
      throw new InfrastructureException(
          ErrorCode.GRAPH_API_ERROR,
          "Erro inesperado ao acessar Graph API para lista='%s' site='%s': %s"
              .formatted(listId, siteId, unexpectedException.getMessage()));
    }
  }
  ...
  private synchronized String getAccessToken() {
    ...
    try {
      ...
      tokenExpiresAt =
          Instant.now().plusSeconds(tokenResponse.expiresIn() - TOKEN_REFRESH_BUFFER_SECONDS);
      log.info(
      ...
      throw new InfrastructureException(
          ErrorCode.GRAPH_UNAUTHORIZED,
          "Falha na autenticação Azure (HTTP %d) — verifique GRAPH_TENANT_ID, GRAPH_CLIENT_ID e GRAPH_CLIENT_SECRET"
              .formatted(authError.getStatusCode().value()));
    } catch (InfrastructureException infraError) {
      throw infraError;
  ...
  private String buildUrl(String siteId, String listId, Set<String> requestedFields, int top) {
    ...
    return "%s/sites/%s/lists/%s/items?$expand=%s&$top=%d"
